...
fun PinterestWebViewScreen(
    ...
    class PinterestWebViewScreenState(
    ...
    val screenState = remember { PinterestWebViewScreenState() }

    val scope = rememberCoroutineScope()

    val extractAndSetMediaInfo: (String) -> Unit = { pageUrl ->
        if (pageUrl.contains("pinterest.com") || pageUrl.contains("pin.it")) {
            screenState.isLoadingMedia = true
            scope.launch {
                val cookies = getCookiesForUrl(pageUrl)
                screenState.mediaInfo = pinterestFetcher.fetchMediaDetails(pageUrl, cookies)
                if (screenState.mediaInfo == null || screenState.mediaInfo?.mediaUrl.isNullOrEmpty()) {
                    Toast.makeText(context, "Media link not found on this pin.", Toast.LENGTH_SHORT).show()
                }
    ...
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("Pinterest Pin Viewer") },
                navigationIcon = {
            ...
            screenState.mediaInfo?.let { info ->
                ...
                Box(
                    ...
                    if (screenState.isLoadingMedia) {
                        ...
                        Text(
                            "Navigate to a pin to extract media. Login may be required for private pins.",

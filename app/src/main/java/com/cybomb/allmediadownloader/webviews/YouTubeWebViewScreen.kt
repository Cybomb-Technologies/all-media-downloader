...
fun YouTubeWebViewScreen(
    ...
    // State object for the WebView Screen (State management encapsulation)
    class YouTubeWebViewScreenState(
    ...
    val screenState = remember { YouTubeWebViewScreenState() }
    ...
    val extractAndSetMediaInfo: (String) -> Unit = { pageUrl ->
        if (pageUrl.contains("youtube.com")) {
            screenState.isLoadingMedia = true // Update state object
            scope.launch {
                // Use the cookies that the WebView automatically saves to CookieManager
                val cookies = getCookiesForUrl(pageUrl)

                // Use the fetcher to try and get the media details with cookies
                screenState.mediaInfo = youTubeFetcher.fetchMediaDetails(pageUrl, cookies) // Update state object
                ...
                } else if (screenState.mediaInfo?.type == "Thumbnail (Image)") {
                    Toast.makeText(context, "Only found the thumbnail. Navigate to the video page.", Toast.LENGTH_LONG).show()
                }

                screenState.isLoadingMedia = false // Update state object
            }
        }
    }
    ...
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("YouTube Video Viewer") },
                navigationIcon = {
        ...
        Column(modifier = Modifier.fillMaxSize().padding(padding)) {
            ...
            screenState.mediaInfo?.let { info ->
                ...
                Button(
                    ...
                    enabled = !screenState.isLoadingMedia && info.type != "Thumbnail (Image)" // Uses state object
                ) {
                ...
                Box(
                    ...
                    if (screenState.isLoadingMedia) { // Uses state object
                        CircularProgressIndicator(modifier = Modifier.size(24.dp), strokeWidth = 2.dp)
                    } else {
                        Text(
                            "Navigate the video page above, or log in to access geo-blocked or private content.",
